...
import { calculateSlotsPerDay } from "../../helpers/utils.AvailabilityRules";
...
const TimeSelector = ({ done }) => {
    ...
    const handleStartChange = e => {
        setExtras({
            ...
            limits: {
                ...
                slotsPerDay: calculateSlotsPerDay(
                    extras.limits.duration,
                    e.target.value,
                    extras.limits.end
                )
            }
    ...
    const startOptions = timeOptions
        .filter(time => time < extras.limits.end)
        .map(time => (
    ...
    const endOptions = timeOptions
        .filter(time => time > extras.limits.start)
        .map(time => (
